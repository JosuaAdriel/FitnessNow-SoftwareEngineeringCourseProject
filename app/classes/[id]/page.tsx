...
export default async function Class({ params }: { params: { id: string } }) {
  ...
  const { data: dataUser } = await supabase.from("users").select("activeUntil").eq("id", User.user?.id);
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 mt-8">
        ...
        <div className="detail md:ml-4 justify-center md:max-w-[50%]">
          ...
          <div className="grid grid-cols-2">
            ...
            <p className="text-md mt-5">Time:</p>
            <p className="text-md mt-5">{kelas.time}</p>
            <p className="text-md mt-5">Duration:</p>
            <p className="text-md mt-5">{kelas.duration}</p>
            <p className="font-bold text-red-700 text-2xl my-3">Rp.{kelas.price}</p>
          </div>
          {isRegistered == null && curUser.activeUntil ? <EnrollClassForm classs={kelas} /> : <></>}
        </div>
      </div>
    </div>
  );
